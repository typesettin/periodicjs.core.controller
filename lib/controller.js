...
Controller.prototype.loadModel = function (options) {
	var model = options.model,
		...
		cachetype = global.CoreCache.DataCache.type,
		cachekey = global.CoreCache.generateKeyFromRequestUrl(cachetype,model.modelName+'+'+docid),
		cachedData,
...
Controller.prototype.updateModel = function (options) {
	var model = options.model,
		...
		updateOperation,
		cached = (typeof options.cached === 'boolean' && options.cached===true) && true,
		useCacheTest = (useCache && cached && req.headers.periodicCache!=='no-periodic-cache');
