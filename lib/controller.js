...
Controller.prototype.controller_routes = function(options){
	...
	if(options.use_append_array_data){
		routesObject.append = Controller.prototype.controller_array_append; //loadFullItemData
	}
	if(options.use_remove_array_data){
		routesObject.remove = Controller.prototype.controller_array_remove; //loadFullItemData
	}
...
Controller.prototype.controller_cli = function(options){
	return function (argv) {
		if (argv.search) {
			var Model_To_Search = periodicResources.mongoose.model(options.model_name.capitalizeFirstLetter()),
				query,
				offset = argv.offset,
				sort = argv.sort,
				limit = argv.limit,
	...
	if (err) {
		...
		async.parallel({
			populateItems:function(asyncCB){
				periodicResources.mongoose.model(options.model_name.capitalizeFirstLetter()).populate(doc,{
			...
			if (loadCompilationDataError) {
				...
				async.parallel({
					entity_item_tags: function (callback) {
						periodicResources.mongoose.model(options.model_name.capitalizeFirstLetter()).populate(loadCompilationDataResults.populateItems, {
					...
					entity_collection_tags: function (callback) {
						periodicResources.mongoose.model(options.model_name.capitalizeFirstLetter()).populate(loadCompilationDataResults.populateCollections, {
			...
			if (err) {
				...
				async.parallel({
					tags: function (callback) {
						periodicResources.mongoose.model(options.model_name.capitalizeFirstLetter()).populate(populatedcollection, {
