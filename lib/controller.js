...
Controller.prototype.handleDocumentQueryErrorResponse = function (options) {
	...
	if (req.query.format === 'json' || req.is('json')) {
		res.send({
			'result': 'error',
			'data': {
				error: errormessage
...
var depopulateArray = function(array){
	...
	for(var i=0; i<array.length;i++){
		if(typeof array[i] ==='object' && typeof array[i]._id==='undefined'){
			depopulatedArray[i] = depopulate(array[i]);
		}
		else{
...
var getRoutes = function (options) {
	var router = options.router,
		model = options.model,
		middleware = options.middleware;
	router.route(`/${ model.name_plural }`)
		.get(middleware[`load${ model.page_plural_title }WithCount`], middleware[`load${ model.page_plural_title }WithDefaultLimit`], middleware[`load${ model.page_plural_title }`], middleware.index)
		.post(periodicResources.core.controller.save_revision, middleware.create);
	router.route(`/${ model.name_plural }/:id`)
		.get(middleware[`loadFull${ model.capital_name }`], middleware.show)
		.put(periodicResources.core.controller.save_revision, middleware.update);
	router.get(`/${ model.name_plural }/new`, middleware.new);
	router.get(`/${ model.name_plural }/edit`, middleware.edit);
	router.delete(`/${ model.name_plural }/:id/`, middleware.remove);
	return router;
};

Controller.prototype.controller_routes = function (options) {
	var viewmodel = Controller.prototype.getViewModelProperties(options),
		router = periodicResources.express.Router(),
		routesObject = {
			new: Controller.prototype.controller_new(options),
			show: Controller.prototype.controller_show(options),
			edit: Controller.prototype.controller_edit(options),
...
Controller.prototype.controller_load_model_with_population = function (options) {
	return function(req, res, next){
		...
		Controller.prototype.loadModel({
			...
			docnamelookup: options.docnamelookup,
			model: model_to_use,
			population: (req.controllerData && typeof req.controllerData.skip_population==='boolean' && typeof req.controllerData.skip_population)? '' : options.load_multiple_model_population,
			callback: function (err, doc) {
