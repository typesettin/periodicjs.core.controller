...
Controller.prototype.respondInKind = function (options, asyncCallback) {
	...
	if (typeof responseData !== 'object' || typeof responseData.result !== 'string') {
		responseData = formatJsonResponse(responseData);
	}
...
Controller.prototype.handleDocumentQueryErrorResponse = function (options) {
	var err = options.err,
		...
		req = options.req,
		res = options.res,
		loggerFunction = (options.use_warning===true) ?logger.warn : logger.error,
					...
					if (options.population) {
						model.findOne({
							...
							if (err) {
								...
								Controller.prototype.responseInKind({
									req: req,
									res: res,
									responseData: formatJsonResponse(saveddoc)
								});
...
Controller.prototype.getRoutes = function (options) {
	...
	router.route(`/${ model.name_plural }/:id`)
		...
		.delete((override && override.delete_item && Array.isArray(override.delete_item)) ? override.delete_item : middleware.remove);
