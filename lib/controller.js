...
Controller.prototype.respondInKind=function(options){
	...
	if(path.extname(req.originalUrl)==='.html' || path.extname(req.originalUrl)==='.htm'){
		// console.log('responding because format is html');
		callback(req,res,responseData);
	}
	else if (req.query.format === 'json' || req.params.ext === 'json' || path.extname(req.originalUrl)==='.json') {
		// console.log('responding because format is json');
		res.send(responseData);
	}
	else if (req.query.callback) {
		// console.log('responding because format is jsonp with callback');
		res.jsonp(responseData);
	}
	else if (options.redirecturl) {
		// console.log('responding with redirect');
		res.redirect(options.redirecturl);
	}
	else if (req.is('json') || req.is('application/json')) {
		// console.log('responding because request is a  json request');
			...
			if (err) {
				...
				Controller.prototype.respondInKind({
					...
					callback:function(req,res,responseData){
						// console.log('respondInKind html render view');
...
var returnIsDateDifferent = function(originalDate,revisionDate){
	var checkrevdate = new Date(revisionDate).toString();
	var checkoridate = new Date(originalDate).toString();
	if(checkrevdate==='Invalid Date' || checkoridate==='Invalid Date'){
		return 'not';
	}
	else if(originalDate === revisionDate || originalDate.toString() === revisionDate.toString()){
		return 'same';
	}
	else if(checkoridate !== checkrevdate ){
		// console.log(originalDate,'checkoridate',checkoridate);
		// console.log(revisionDate,'checkrevdate',checkrevdate);
		// console.log('returnIsDateDifferent');
		return 'diff';
	}
	else{
		return 'not';
...
var returnIsAssociatedDataDifferent = function(originalData,revisionData){
	if((revisionData && revisionData._id )|| (originalData && originalData._id)){
		if(originalData && originalData._id && originalData._id.toString() === revisionData.toString()){
			return 'same';
		}
		else if(revisionData && revisionData._id && revisionData._id.toString() === originalData.toString() ){
			return 'same';
		}
		else {
			// console.log('returnIsAssociatedDataDifferent is true');
...
var returnIsArrayDifferent = function(originalArray,revisionArray){
	var returnval = 'not';
	console.log('Array.isArray(revisionArray)',Array.isArray(revisionArray));
	if(Array.isArray(originalArray) && originalArray.length===1 && originalArray[0]._id && !Array.isArray(revisionArray)){
		return returnIsAssociatedDataDifferent(originalArray[0],revisionArray);
	}
	else if(!Array.isArray(revisionArray)){
		// console.log('revisionArray',revisionArray);
		returnval =  'not';
	}
	else if(revisionArray.length !== originalArray.length){
		// console.log('Object.keys(originalArray).length',Object.keys(originalArray).length);
		// console.log('Object.keys(revisionArray).length',Object.keys(revisionArray).length);
		returnval =  'diff';
	}
	else if( JSON.stringify(revisionArray) === JSON.stringify(originalArray) ){
		returnval =  'same';
	}
	else if(revisionArray.length > 0){
		var loopreturnval ='not';
		for(var i in revisionArray){
			if(loopreturnval==='diff'){
				return loopreturnval;
			}
			if(originalArray[i] && originalArray[i].toString() === revisionArray[i].toString()){
				loopreturnval =   'same';
			}
			else if(typeof originalArray[i]==='undefined' && typeof revisionArray[i]!=='undefined'){
				// console.log('originalArray[i]',i,'is undefined');
				loopreturnval =   'diff';
			}
			else{
				// console.log('array i',i,'is different');
				loopreturnval =  (returnIsDateDifferent(originalArray[i],revisionArray[i]) || returnIsAssociatedDataDifferent(originalArray[i],revisionArray[i]));
			}
		}
	}
	else{
...
var returnIsObjectDifferent = function(originalObject,revisionObject){
	var returnval = 'not';
	// console.log('typeof revisionObject',typeof revisionObject);
	// console.log('revisionObject',revisionObject);
	// console.log('typeof originalObject',typeof originalObject);
	if (typeof revisionObject !=='object' || typeof originalObject !=='object'){
		returnval =  'not';
	}
	else if ( Object.keys(originalObject).length !== Object.keys(revisionObject).length  ){
		// console.log('Object.keys(originalObject).length',Object.keys(originalObject).length);
		// console.log('Object.keys(revisionObject).length',Object.keys(revisionObject).length);
		returnval =  'diff';
	}
	else if(Object.keys(revisionObject).length > 0){
		var loopreturnval ='not';
		for(var keyindex =0; keyindex < Object.keys(revisionObject).length; keyindex++){
			var key = Object.keys(revisionObject)[keyindex];
			// console.log('checking key',key);
			if(loopreturnval==='diff'){
				return loopreturnval;
			}
			else{
				if(originalObject[key] && JSON.stringify(originalObject[key]) === JSON.stringify(revisionObject[key])){
					// console.log('JSON.stringify(revisionObject[key])',JSON.stringify(revisionObject[key]));
					// console.log('JSON.stringify(originalObject[key])',JSON.stringify(originalObject[key]));
					// console.log('obj is same has same val');
					loopreturnval =  'same';
				}
				else if(typeof originalObject[key]==='undefined' && typeof revisionObject[key]!=='undefined'){
					// console.log('!originalObject[key]',key,originalObject[key]);
					loopreturnval =  'diff';
				}
				// else if(originalObject[key]!==null && revisionObject[key]!==null && originalObject[key]!==undefined && revisionObject[key]!==undefined &&originalObject[key].toString()!==revisionObject[key].toString() ){
				// 	console.log('obj exts but not same');
				// 	loopreturnval =  'diff';
				// }
				else if(typeof revisionObject[key]==='string' && originalObject[key] && revisionObject[key].toString()!==originalObject[key].toString()){
					// console.log('obj has diff stringvals');
					loopreturnval =  'diff';
				}
				else if(originalObject[key]===null && (revisionObject[key]==='' || revisionObject[key]===null || revisionObject[key]===undefined)){
					// console.log('JSON.stringify(revisionObject[key])',JSON.stringify(revisionObject[key]));
					// console.log('JSON.stringify(originalObject[key])',JSON.stringify(originalObject[key]));
					// console.log('obj nul val is the same');
					loopreturnval =  'same';
				}
				else if(originalObject[key]===revisionObject[key] ){
					// console.log('obj has same value');
					loopreturnval =  'same';
				}
				else if(originalObject[key]!==null && revisionObject[key]!==null && originalObject[key]!==undefined && revisionObject[key]!==undefined &&originalObject[key].toString()===revisionObject[key].toString() ){
					// console.log('obj nul val check is the same');
					loopreturnval =  'same';
				}
				else{
					// console.log('object key',key,'typeof originalObject[key]',typeof originalObject[key],originalObject[key]);
					loopreturnval = (returnIsDateDifferent(originalObject[key],revisionObject[key]) || 
						returnIsAssociatedDataDifferent(originalObject[key],revisionObject[key]) || 
						returnIsArrayDifferent(originalObject[key],revisionObject[key]));
				}
			}
		}
	}
	else if (JSON.stringify(originalObject) !== JSON.stringify(revisionObject)) {
		returnval = 'diff';
	}
	else{
		returnval = 'same';
...
var returnObjectDifference = function (original, revision) {
	...
	if(original.toObject && typeof original.toObject==='function'){
		original = original.toObject();
	}
	if(revision.toObject && typeof revision.toObject==='function'){
		revision = revision.toObject();
	}
	for (var prop in revision) {
		var dateisdiff='not', objisdiff='not', arrayisdiff='not';
		console.log('        ');
		console.log('------------------------');
		console.log('checking prop',prop);
		if( JSON.stringify(original[prop])===JSON.stringify(revision[prop])){
			comparestatus = ' prop(' + prop + ') is the same';
		}
		else if(typeof original[prop] ==='undefined'){
			comparestatus = 'Original prop(' + prop + ') is missing';
			changesetdata[prop] = original[prop];
		}
		else {
			dateisdiff = returnIsDateDifferent(original[prop],revision[prop]);
			if(dateisdiff==='diff'){
				comparestatus = 'Date prop(' + prop + ') is different';
				// console.log('revision[prop]',revision[prop]);
				// console.log('original[prop]',original[prop]);
				changesetdata[prop] = original[prop];
			}
			else{
				objisdiff = returnIsObjectDifferent(original[prop],revision[prop]);
				if(objisdiff==='diff'){
					comparestatus = 'Object prop(' + prop + ') is different';
					// console.log('revision[prop]',revision[prop]);
					// console.log('original[prop]',original[prop]);
					changesetdata[prop] = original[prop];
				}
				else{
					arrayisdiff = returnIsArrayDifferent(original[prop],revision[prop]);
					 if(arrayisdiff==='diff'){
			...
			if(dateisdiff==='not' && objisdiff==='not' && arrayisdiff==='not' && JSON.stringify(original[prop])!==JSON.stringify(revision[prop])){
				// console.log('JSON.stringify(original[prop])',JSON.stringify(original[prop]));
				// console.log('JSON.stringify(revision[prop])',JSON.stringify(revision[prop]));
				comparestatus = ' prop(' + prop + ') is the diff';
				changesetdata[prop] = original[prop];
			}
			else if(dateisdiff==='diff' || objisdiff==='diff' || arrayisdiff==='diff'){
				// console.log('JSON.stringify(original[prop])',JSON.stringify(original[prop]));
				// console.log('JSON.stringify(revision[prop])',JSON.stringify(revision[prop]));
				changesetdata[prop] = original[prop];
			}
			console.log('dateisdiff',dateisdiff);
			console.log('objisdiff',objisdiff);
			console.log('arrayisdiff',arrayisdiff);
		}
...
Controller.prototype.controller_routes = function(options){
	var viewmodel = Controller.prototype.getViewModelProperties(options),
		routesObject = {
			new: Controller.prototype.controller_new(options),
			...
			searchResults: Controller.prototype.controller_search_index(options)
		};
	...
	if(options.use_taxonomy_controllers){
		...
		routesObject.loadChildren = Controller.prototype.controller_load_model_children(options);
		routesObject.showChildren = Controller.prototype.controller_show_model_children(options);
	}
	else{
	...
	if(options.use_append_array_data){
		routesObject.append = Controller.prototype.controller_array_append(options); //loadFullItemData
	}
	if(options.use_remove_array_data){
		routesObject.remove = Controller.prototype.controller_array_remove(options); //loadFullItemData
	}
...
Controller.prototype.controller_update = function (options) {
	return function(req, res){
		...
		if(options.model_name === 'user'){
			...
			// console.log('user_controller_model_data',user_controller_model_data);
