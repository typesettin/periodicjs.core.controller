...
Controller.prototype.controller_update = function(options) {
	return function(req, res) {
		...
		// console.log('UPDATE CONTROLLER ', { update_controller_model })
		// let depopulateProps = options.load_model_population.split(' ');
		// depopulateProps.forEach(prop => {
		//     update_controller_model.depopulate(prop);
		// })
		...
		if (!err) {
			Controller.prototype.updateModel({
				cached: req.headers.periodicCache !== 'no-periodic-cache',
				model: model_to_use,
				id: update_controller_model.docid || req.params.id,
...
Controller.prototype.controller_load_model = function(options) {
	return function(req, res, next) {
		...
		var fields = (options.fields && typeof options.fields === 'object') ? options.fields : undefined;
...
Controller.prototype.controller_model_search_query = function(options) {
	...
	if (options.controllerOptions && options.controllerOptions.model_type && options.controllerOptions.model_name && options.controllerOptions.model_type === 'sequelize') {
		model_to_use = options.controllerOptions.sequelizeDB.models[options.controllerOptions.model_name];
		// docnamelookup: options.docnamelookup,
		// model_type: options.model_type,
		// model_name: options.model_name,
		// sequelizeDB: options.sequelizeDB,
			...
			if (err) {
				...
				parallelTask[viewmodel.page_plural_count] = function(cb) {
					if (req.headers['load' + viewmodel.page_single_count]) { // loaditemcount
						if (options.controllerOptions && options.controllerOptions.model_type && options.controllerOptions.model_name && options.controllerOptions.model_type === 'sequelize') {
							model_to_use.count({ where: query })
								.then((count) => {
									cb(null, count);
								})
								.catch((e) => {
				...
				parallelTask[viewmodel.page_plural_query] = function(cb) { // itemsquery
					Controller.prototype.searchModel({
						...
						offset: offset,
						population: population,
						model_type: options.controllerOptions.model_type,
						return_raw: options.controllerOptions.return_raw,
						model_name: options.controllerOptions.model_name,
						sequelizeDB: options.controllerOptions.sequelizeDB,
				...
				async.parallel(
					...
					function(err, results) {
						if (err) {
							...
							// if(options.controllerOptions && options.controllerOptions.model_type && options.controllerOptions.model_name && options.controllerOptions.model_type==='sequelize'){
							// 	req.controllerData[viewmodel.name_plural] = results[viewmodel.page_plural_query].map((view_model_sequelize_row)=>  view_model_sequelize_row.dataValues);
							// }
							// else{
